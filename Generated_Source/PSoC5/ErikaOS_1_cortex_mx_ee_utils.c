...
#if (Number_of_ISRs > 1) && defined isr_Reset__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_Reset__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_Reset;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_Reset__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_Reset__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_Reset__INTC_NUMBER);
#elif Number_of_ISRs > 1
#error  The Interrupt isr_Reset could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_Reset */
