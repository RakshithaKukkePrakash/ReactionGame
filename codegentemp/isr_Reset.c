...
{
    #ifdef isr_Reset_INTERRUPT_INTERRUPT_CALLBACK
        isr_Reset_Interrupt_InterruptCallback();
    #endif /* isr_Reset_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void isr_Reset_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_Reset__INTC_NUMBER] = address;
}
...
cyisraddress isr_Reset_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_Reset__INTC_NUMBER];
}
...
*
*   Note calling isr_Reset_Start or isr_Reset_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after isr_Reset_Start or isr_Reset_StartEx has been called. 
...
{
    *isr_Reset_INTC_PRIOR = priority << 5;
}
...
uint8 isr_Reset_GetPriority(void)
{
    ...
    priority = *isr_Reset_INTC_PRIOR >> 5;
